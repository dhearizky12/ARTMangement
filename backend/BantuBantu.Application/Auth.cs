...
public record UserDto(Guid Id, string Email, string FullName, string? PictureUrl, string Role, bool ProfileCompleted, string ProfileStep)
{
    public static UserDto From(User u) => new(u.Id, u.Email, u.FullName, u.PictureUrl, u.Role.ToString(), u.ProfileCompleted, u.ProfileCompleted ? "done" : u.ProfileStep);
}
...
public class AuthService(IAuthRepository repository, IGoogleIdentityVerifier google, ITokenService tokens, IPasswordService passwords) : IAuthService
{
    public async Task<AuthResult> GoogleAsync(string credential, CancellationToken ct)
    {
        ...
        if (user.Role != UserRole.User) throw new AuthenticationFailedException();
    ...
    public async Task<AuthResult> AdminAsync(string email, string password, CancellationToken ct)
    {
        ...
        if (!passwords.Verify(user ?? new User(), password) || user is null) throw new AuthenticationFailedException();
        return await IssueAsync(user, ct);
    }
